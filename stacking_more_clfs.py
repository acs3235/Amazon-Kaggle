...
clf1 = xgb.XGBClassifier(max_depth=18, colsample_bytree=0.45, n_estimators = 99, random_seed = 10)
clf2 = AdaBoostClassifier()
clf3 = LogisticRegression(penalty='l2',C=3)
clf4 = xgb.XGBClassifier(max_depth=18, colsample_bytree=0.45, n_estimators = 99, random_seed = 1)
clf5 = xgb.XGBClassifier(max_depth=18, colsample_bytree=0.45, n_estimators = 99, random_seed = 5)
clf6 = xgb.XGBClassifier(max_depth=18, colsample_bytree=0.45, n_estimators = 99, random_seed = 30)
...
#model4
model4 = clf4.fit(fold1_X_encoded, fold1_y)
model4_p = model4.predict_proba(fold2_X_encoded)[:, 1]
#model5
model5 = clf5.fit(fold1_X_encoded, fold1_y)
model5_p = model5.predict_proba(fold2_X_encoded)[:, 1]
#model6
model6 = clf6.fit(fold1_X_encoded, fold1_y)
model6_p = model6.predict_proba(fold2_X_encoded)[:, 1]
...
m4 = clf4.fit(train_X_encoded,train_y)
m5 = clf5.fit(train_X_encoded,train_y)
m6 = clf6.fit(train_X_encoded,train_y)
...
m4p = m4.predict_proba(test_X_encoded)[:, 1]
m5p = m5.predict_proba(test_X_encoded)[:, 1]
m6p = m6.predict_proba(test_X_encoded)[:, 1]
...
submission.to_csv("stacking_more_clfs.csv", index=False)